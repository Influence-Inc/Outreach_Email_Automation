...
function renderStatusCell(r, cell) {
  ...
  if (r.contract && r.contract.url) {
    ...
    if (signed) {
      ...
      // Straight PDF of the contract the creator signed HERE — the same terms
      // the signing page showed, plus the details they filled in and the
      // signature they drew. Generated by this backend from the contract row,
      // so unlike "Contract" above it needs no Creator-DB round-trip and works
      // the moment the creator signs. This is the copy to send when a creator
      // asks for "a copy of what I signed"; account and tax identifiers on it
      // are masked to their last four digits.
      const pdf = document.createElement('button');
      pdf.type = 'button';
      pdf.className = 'ghost small cr-signed-pdf';
      pdf.textContent = 'Signed PDF';
      pdf.title = 'Download the signed agreement as a PDF (safe to send to the creator)';
      pdf.onclick = () => {
        window.open('/api/contract-pdf/' + encodeURIComponent(r.contract.token), '_blank', 'noopener');
      };
      left.appendChild(pdf);
    } else {
      const copy = document.createElement('button');
      copy.type = 'button';
