...
-- when the reply came in by reading the message (negotiation.judgeSender asks
-- Claude; services/salutation.js pattern-matches as the fallback). It is what
-- licenses our emails to talk about the creator in the third person. NOT
-- inferred from the sending address, which settles this far less often than it
-- looks — managers routinely reply from the creator's own inbox. Stored rather
-- than re-derived, because by the time an offer email goes out the inbound is
-- gone and a name-vs-name comparison would call every short form ("Kam" for
-- "Kamran") a third party. NULL on rows written before this column existed.
