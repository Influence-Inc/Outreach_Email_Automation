...
function askedForReferences(text) {
  ...
  const patterns = [
    ...
    // with" / "brands you've worked with" / "creators you collaborated with" —
    // the "you" requirement is what excludes the creator describing THEIR OWN
    // history ("I've worked with other creators", "we've worked with brands
    // before"). The verb tense is left loose (worked / collaborated / partnered,
    // with or without "'ve"/"have") so "what are the previous creators you
    // collaborated with" reads as a portfolio ask too.
    /\b(?:have\s+you|you(?:'?ve|\s+(?:have|had))?)\s+(?:worked|partnered|collaborated)\s+with\b/i,
  ];
...
// Look up the team-approved rewritten master prompt for a single reply type
// (e.g. "reply1", "reply2", "answer_question"). Returns '' when unset or when
// overrides aren't in context, so callers can safely concatenate the result.
// This is what the Guidelines UI generated by feeding the admin's plain-
// English instruction + the current effective master prompt through Claude,
// then persisted on Confirm — so a match means "the team has revised the
// master prompt for this reply and this is the version to follow".
function replyOverrideFor(ctx, key) {
  const overrides = ctx && ctx.replyOverrides;
  if (!overrides || typeof overrides !== 'object') return '';
  const v = overrides[key];
  return typeof v === 'string' ? v.trim() : '';
}

// Render a single reply-type's override as an authoritative prompt block that
// supersedes the canonical template / rule shown earlier in the system prompt.
// Used by the offer / counter-offer prompts where the whole prompt writes ONE
// reply type. Empty string when unset (the base template + rule stand as-is).
function singleReplyOverrideBlock(ctx, key, label) {
  const override = replyOverrideFor(ctx, key);
  if (!override) return '';
  return [
    '',
    `TEAM-REVISED MASTER PROMPT for THIS email (${label}) — this is the authoritative version. Follow it verbatim in preference to the canonical template / rules shown earlier. If it changes wording of the canonical template, use the revised wording; if it removes or adds a section, honor that; on any conflict with earlier prompt text, the revised master prompt below wins.`,
    '"""',
    override,
    '"""',
    '',
...
// Render every reply-type override that has content as an action-keyed block.
// Used by the classifier prompt (handleCreatorReply), which picks ONE action
// out of many and writes the reply inline — Claude reads the map and follows
// the revised master prompt whose key matches the action it chose. Empty
// string when no overrides are set.
function actionReplyOverridesBlock(ctx) {
  const overrides = ctx && ctx.replyOverrides;
  if (!overrides || typeof overrides !== 'object') return '';
  const entries = (REPLY_NOTE_TYPES || [])
    .map((t) => ({ key: t.key, label: t.label, override: replyOverrideFor(ctx, t.key) }))
    .filter((e) => e.override);
  if (!entries.length) return '';
  const lines = [
    '',
    'TEAM-REVISED MASTER PROMPTS for specific reply types — when the action you choose below matches one of these keys, the revised master prompt is the authoritative version to follow; it supersedes the canonical template / rule shown earlier in this system prompt. If it changes wording, use the revised wording; if it removes or adds a section, honor that.',
  ];
  for (const e of entries) {
    lines.push(`• [${e.key}] ${e.label}:`);
    lines.push('"""');
    lines.push(e.override);
    lines.push('"""');
    lines.push('');
  }
  return lines.join('\n');
...
async function handleCreatorReply(creator, replyText, ctx) {
  ...
  const system = [
    ...
    `- "asking_details": the creator is interested but has not yet seen the standard collab pitch. Use this for the FIRST substantive reply when we have not yet sent REPLY 1 (current stage is the opening — no negotiation email has gone out). Write the email by ADAPTING REPLY 1 (brand "${v.brandName}", sign "- ${v.managerName}"). In Timelines, propose the cadence "${v.cadence}" and an approximate posted-by date you compute from today's date for a 2-video package. Include the "Past content references" section ONLY if the sender explicitly asked to see examples of past work / a portfolio / other creators we've worked with — otherwise drop that whole section from your adaptation. send_now=true. quoted_rate=null. IMPORTANT: if we have ALREADY sent REPLY 1 (any stage past the opening) and the creator is only asking to SEE our references / past work / previous creators we've collaborated with, that is NOT "asking_details" — do NOT re-send the details pitch. Use "answer_question" and share only the reference accounts.`,
    `- "answer_question": the creator asked a specific factual question about an already-discussed deal. Common topics that ARE safe to answer from the REPLY 1 / REPLY 2 templates and the campaign context above: posting platform (Instagram only, no TikTok/YouTube cross-posting in this deal), content format (Reels), posting cadence ("${v.cadence}"), approximate timeline / posted-by date, creative freedom (yes, no script approval required), exclusivity (none), what we need from them (their rate, then we share a tailored offer; once accepted, posting can begin), who Influence is (a brand-partnerships team), our references / past work / previous creators we've collaborated with (when the sender asks to see examples of past work / a portfolio / references / who we've worked with, share the reference accounts (${v.refs}) directly — this is a benign question to answer here, never a reason to hand off, even after REPLY 1 has been sent), payment timing (per the "Payment details" block in REPLY 2: after the post is up and verified). Write a SHORT reply that (1) directly answers their question in 1-3 sentences using ONLY facts from the templates / campaign context above or facts our team already stated in the example exchanges shown before this message, then (2) one short follow-up line keeping the negotiation moving that FITS the current stage and what has already been discussed in this thread — if we are still pre-rate (no rate has ever been shared, quoted, or offered), ask for their rate; if an offer is on the table awaiting their decision (stage AWAITING_DECISION, or an approved offer is present in this context), nudge ONLY for their DECISION on that pending offer; if a rate has already been shared/quoted/offered/agreed at any point, treat rate discussion as CLOSED ground — do NOT re-open it, do NOT invite them to "chat rates" / "discuss rates" / "share your rate" / anything equivalent, and instead either nudge on the pending decision (if one is pending) or close warmly with a simple sign-off that opens no new topic. Rate re-invitations after a rate is already on record are a hard error — when in doubt, close warmly without a follow-up ask. Sign "- ${v.managerName}". send_now=true. quoted_rate=null. NEVER invent specifics that are not in the campaign context, templates, past example exchanges, or already-quoted offer — if you would have to guess a number, a date beyond what cadence-math gives you, or any term not covered by those sources, use "escalate" instead.`,
...
async function draftOfferEmail(
  ...
  const system = [
    ...
    guidelinesBlock(ctx),
    revised
      ? singleReplyOverrideBlock(ctx, 'counter_offer', 'Counter-offer — revised numbers')
      : singleReplyOverrideBlock(ctx, 'reply2', 'Reply 2 — priced offer'),
...
async function buildOfferDraft(creatorId, { offer: rawOffer, instructions = '' } = {}) {
  ...
  const [guidelines, replyNotes, replyOverrides] = await Promise.all([
    getGuidelines(),
    getReplyPromptNotes(),
    getReplyPromptOverrides(),
  ]);
...
async function processReply(creatorId) {
  ...
  if (result.action === 'asking_details' && creator.negotiation_status) {
    // Exception: a request to SEE our references / past work / previous creators
    // we've collaborated with is a benign question, not a fresh intro — even
    // though Claude (or the heuristic) tends to route it to REPLY 1 because that
    // is where the reference list lives. Answer it directly by sharing ONLY the
    // reference accounts, preserving the current stage. This avoids both the
    // wrongful hand-off AND the "we re-sent the whole details pitch" bug: we
    // never re-send REPLY 1, we just share the references the creator asked for.
    if (includeRefs) {
      console.log(
        `[negotiation] creator ${creator.id}: reference request after REPLY 1 (stage ${creator.negotiation_status}) — sharing references directly instead of delegating`,
      );
      await applyReply(creator, ctx, {
        action: 'answer_question',
        email: templates.referencesReply(templateVars(ctx)),
        send_now: true,
        quoted_rate: null,
      });
      await markHandled();
      return { action: 'answer_question', reason: 'shared_references' };
    }
    console.log(
